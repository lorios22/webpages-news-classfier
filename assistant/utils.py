...
def clean_and_structure_content(content: str) -> Dict:
    ...
    try:
        ...
        is_news_article = any(indicator in text.lower() 
                            for indicator in ["news", "report", "announced"])
        is_blog_post = any(indicator in text.lower() 
                          for indicator in ["blog", "opinion", "thoughts"])
...
class ScoringPitfallsValidator:
    ...
    def __init__(self):
        self.score_boundaries = {
            "misinformation": {
                (1, 2): "Distinguish between completely false (1) and partially misleading (2)",
