                ...
                if self.context_engine:
                    try:
                        context_analysis = self.context_engine.analyze_context(
                            article["content"], article.get("category", "unknown")
                        )
    ...
    async def generate_comprehensive_outputs(self, processed_articles: List[Dict[str, Any]]) -> Dict[str, str]:
        ...
        try:
            ...
            with open(report_file, "w", encoding="utf-8") as f:
                ...
                f.write(
                    f"- **Total Execution Time:** {(datetime.now() - self.stats['execution_start']).total_seconds():.1f} seconds\n"
                )
                f.write(
                    f"- **Articles per Second:** {len(processed_articles) / processing_time if processing_time > 0 else 0:.2f}\n\n"
                )
